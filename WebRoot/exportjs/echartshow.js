...
echartshow.addTbody = function(jdat,bodyColName,bodyColUnit,top,type){
	...
	for(var i=0;i<len;i++){
		...
		var tr = echartshow.addTrContext('',bodyList[i],bodyColName,bodyColUnit,queryConditionsObj,queryType,queryCondition,clazz) ;
		tbody.append(tr);
...
*/
		if(i==0 && drill==true){
			td = echartshow.addHrefTd(tdClazz, datvalue, "ablue", drillUrl);
...
echartshow.addHrefTd = function(clazz,innerHTML,aclazz,url){
	return echartshow.addTd(clazz,"").append($("<a/>").attr({"title":'下钻报表'}).bind("click",function(){report.drillReport(url);}).addClass(aclazz).css("cursor","pointer").html(innerHTML)) ;
};

echartshow.addLogQuery = function(clazz,innerHTML,aclazz,url,logQueryObject){
	var logQueryParams=null;
	return echartshow.addTd(clazz,"").append($("<a/>").attr({"title":'查询日志',target:"_blank",href:'#?'+logQueryParams}).addClass(aclazz).css("cursor","pointer").html(innerHTML)) ;
};
/**
