...
highcharts.loadReportData = function(url,top,isExistTable){
	...
	simsuperiorReport.targetIp = targetId.replace(/[_]/g, ".");
	$.getJSON(encodeURI(url + "&ip=" + simsuperiorReport.targetIp),{_time:new Date().getTime()},function(result){
...
highcharts.createSpline = function(container,data,timeType){
	...
	$("#"+container).highcharts( {
	    ...
	    tooltip: {
	    	formatter: function() {
	    		var tempname=(this.series.name==null||this.series.name==undefined
...
highcharts.modifyTableInfo = function(moduleId,data,top,type){
	...
	var sheader = highcharts.addThead(header);
...
highcharts.addHrefTd = function(clazz,innerHTML,aclazz,url){
	return highcharts.addTd(clazz,"").append($("<a/>").bind("click",function(){simsuperiorReport.drillReport(url);}).addClass(aclazz).css("cursor","pointer").html(innerHTML)) ;
};

highcharts.addMergeTd = function(column,moreUrl){
	var trStr= highcharts.addTr("right","tableOddTd");
	trStr.append($("<td/>").attr("colspan",column).append($("<a/>").bind("click",function(){simsuperiorReport.moreReport(moreUrl);}).addClass("").css("cursor","pointer").html("更多") ));
	return trStr;
};
