 ...
 */
public final class ReportDataManagerHandler {
	...
	public void removeDatByDatasource(SimDatasource datasource) {
	...
	public List<String> getDelSqlList(SimDatasource datasource,ReportQuery reportQuery){
	...
	private List<String> getAllRelevanceTables(SimDatasource datasource,ReportQuery reportQuery) {
		List<String> relevanceTables=null;
		if (null == datasource) {
			return relevanceTables;
		}
		String type=datasource.getSecurityObjectType();
		if(type.startsWith("Monitor"))
			type="MONITOR";
		List<String>tables=reportQuery.findTableNameListByType(type);
	...
	private String getDelSql(String tableName,Long resourceId){
		StringBuffer sql=new StringBuffer(DEL_SQL);
		sql.append(tableName).append(CONDITION_SQL).append(resourceId);
		return sql.toString();
	}
	public String[] getRelevanceTables(String tableName){
...
//		tables[6]=tables[5]+TMP;
	...
	public void removeHistoryByDatasource(SimDatasource datasource) {
		ReportQuery reportQuery=(ReportQuery)SpringContextServlet.springCtx.getBean("reportQuery");
	...
	public List<String> getDeleteHistorySqlList(SimDatasource datasource,ReportQuery reportQuery){
		List<String> sqlList=null;
	...
	public String getDeleteHistoryName(String tableName){
		if (GlobalUtil.isNullOrEmpty(tableName)
	...
	private String getDeleteHistorySql(String tableName,Long resourceId,int daylong){
	...
	private int timeUnitToDayNum(String keepTime){
		int defaultTime=90;
